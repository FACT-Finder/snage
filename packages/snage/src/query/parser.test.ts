...
describe('parser', () => {
    test('status', () => {
        expect(parser('booleanName absent')).toEqual(right({field: 'booleanName', op: StatusOP, value: StatusValue.Absent}));
        expect(parser('booleanName present')).toEqual(right({field: 'booleanName', op: StatusOP, value: StatusValue.Present}));
        ...
        expect(parser('booleanName = xxx')).toEqual(left({expected: ["'false'", "'true'"], index: {column: 15, line: 1, offset: 14}}));
    });
    test('boolean', () => {
    ...
    test('enum', () => {
        ...
        expect(parser('enumName = xxx')).toEqual(left({expected: ["'abc'", "'cde'"], index: {column: 12, line: 1, offset: 11}}));
    });
    test('string', () => {
        ...
        expect(parser('stringName = ,.><=+\\|!@#$%^&*{}+_-?')).toEqual(right({field: 'stringName', op: '=', value: ',.><=+\\|!@#$%^&*{}+_-?'}));
    ...
    test('semver', () => {
        ...
        expect(parser('semverName <= 1.1.0-100')).toEqual(right({field: 'semverName', op: '<=', value: '1.1.0-100'}));
        expect(parser('semverName >= 1.1.0-SNAPSHOT')).toEqual(right({field: 'semverName', op: '>=', value: '1.1.0-SNAPSHOT'}));
        expect(parser('semverName = aoeuoaeu')).toEqual(left({expected: ['semver'], index: {column: 14, line: 1, offset: 13}}));
    });
    test('ffversion', () => {
        expect(parser('ffVersionName >= 1.1.0-100')).toEqual(right({field: 'ffVersionName', op: '>=', value: '1.1.0-100'}));
        expect(parser('ffVersionName <= 1.1.0-100')).toEqual(right({field: 'ffVersionName', op: '<=', value: '1.1.0-100'}));
        expect(parser('ffVersionName >= 1.1.0-SNAPSHOT')).toEqual(right({field: 'ffVersionName', op: '>=', value: '1.1.0-SNAPSHOT'}));
    ...
    test('multiple', () => {
        ...
        expect(parser('semverName >= 1.1.1 and semverName <= 1.5.5')).toEqual(
            right([{field: 'semverName', op: '>=', value: '1.1.1'}, 'and', {field: 'semverName', op: '<=', value: '1.5.5'}])
        );
    });
    test('braces', () => {
        ...
        expect(parser('(booleanName = false and stringName = abcde)')).toEqual(
            right([{field: 'booleanName', op: '=', value: false}, 'and', {field: 'stringName', op: '=', value: 'abcde'}])
        );
