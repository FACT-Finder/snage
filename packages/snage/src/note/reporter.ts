...
export const stringifyErrors = (mode: ReportMode) => (errors: ValidationError[]): string[] =>
    errors.map(stringifyError(mode));

const stringifyError = (mode: ReportMode) => ({message, context, value}: ValidationError): string => {
